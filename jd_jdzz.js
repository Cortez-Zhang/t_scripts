...
============小火箭=========
京东赚赚 = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_jdzz.js, cronexpr="0 0 * * *", timeout=3600, enable=true
 */
const $ = new Env('京东赚赚');
...
!(async () => {
  $.tuanList = []
  await requireConfig();
  if (helpAuthor) await getAuthorShareCode('https://baidu.com/raw/main/jd_zz1.json');
  if (helpAuthor) await getAuthorShareCode('https://baidu.com/raw/master/jd_zz1.json');
  if (!cookiesArr[0]) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://baidu.com/read/${randomCount}/`}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
